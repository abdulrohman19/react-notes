...
const NotesInput = ({ addNewNote, closeModal }) => {
    ...
    const onTitleChange = (event) => {
        ...
        if (event.target.value.length <= 50) {
            ...
            toast.error('Max length for notes body is 50 word', { toastId: toastId1 });
        }
    }
    ...
    const onSubmitForm = (event) => {
        event.preventDefault();
        if (formData.title === '') {
            toast.error('The Title cannot be empty!', { toastId: toastId2 });
        } else if (formData.noteBody === '') {
            toast.error('Notes body cannot be empty!', { toastId: toastId3 });
        } else {
            const newData = {
                id: +new Date(),
            ...
            if (!result.error) {
                ...
                toast.error('New notes failed to save!');
            }
        }
    }
